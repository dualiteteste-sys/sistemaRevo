/*
  # [MIGRATION] CRUD de Parceiros (Pessoas) — Adiciona campos de contato
  Ajustes:
  - Adiciona suporte para `celular`, `site`, `contato_tags` na RPC `create_update_partner`.
*/

-- Create/Update transacional (SECURITY DEFINER)
...
begin
  ...
  if v_pessoa_id is null then
    insert into public.pessoas (
      empresa_id, tipo, tipo_pessoa, nome, fantasia, doc_unico, rg, carteira_habilitacao,
      email, telefone, celular, site,
      contribuinte_icms, inscr_estadual, isento_ie, inscr_municipal,
      observacoes, contato_tags, codigo_externo
    )
    values (
      v_empresa_id,
  ...
  if v_has_enderecos then
    for v_endereco in select * from jsonb_array_elements(coalesce(v_enderecos_payload, '[]'::jsonb)) loop
      if nullif(v_endereco->>'id','')::uuid is not null then
        update public.pessoa_enderecos set tipo_endereco = v_endereco ->> 'tipo_endereco', logradouro = v_endereco ->> 'logradouro', numero = v_endereco ->> 'numero', complemento = v_endereco ->> 'complemento', bairro = v_endereco ->> 'bairro', cidade = v_endereco ->> 'cidade', uf = v_endereco ->> 'uf', cep = v_endereco ->> 'cep', pais = v_endereco ->> 'pais', updated_at = now() where id = (v_endereco ->> 'id')::uuid and pessoa_id = v_pessoa_id and empresa_id = v_empresa_id;
        v_endereco_ids_in_payload := array_append(v_endereco_ids_in_payload, (v_endereco ->> 'id')::uuid);
      else
        insert into public.pessoa_enderecos (id, empresa_id, pessoa_id, tipo_endereco, logradouro, numero, complemento, bairro, cidade, uf, cep, pais) values (coalesce(nullif(v_endereco->>'id','')::uuid, gen_random_uuid()), v_empresa_id, v_pessoa_id, v_endereco ->> 'tipo_endereco', v_endereco ->> 'logradouro', v_endereco ->> 'numero', v_endereco ->> 'complemento', v_endereco ->> 'bairro', v_endereco ->> 'cidade', v_endereco ->> 'uf', v_endereco ->> 'cep', v_endereco ->> 'pais');
      end if;
    end loop;
  ...
  if v_has_contatos then
    for v_contato in select * from jsonb_array_elements(coalesce(v_contatos_payload, '[]'::jsonb)) loop
      if nullif(v_contato->>'id','')::uuid is not null then
        update public.pessoa_contatos set nome = v_contato ->> 'nome', email = v_contato ->> 'email', telefone = v_contato ->> 'telefone', cargo = v_contato ->> 'cargo', observacoes = v_contato ->> 'observacoes', updated_at = now() where id = (v_contato ->> 'id')::uuid and pessoa_id = v_pessoa_id and empresa_id = v_empresa_id;
        v_contato_ids_in_payload := array_append(v_contato_ids_in_payload, (v_contato ->> 'id')::uuid);
      else
        insert into public.pessoa_contatos (id, empresa_id, pessoa_id, nome, email, telefone, cargo, observacoes) values (coalesce(nullif(v_contato->>'id','')::uuid, gen_random_uuid()), v_empresa_id, v_pessoa_id, v_contato ->> 'nome', v_contato ->> 'email', v_contato ->> 'telefone', v_contato ->> 'cargo', v_contato ->> 'observacoes');
      end if;
    end loop;
