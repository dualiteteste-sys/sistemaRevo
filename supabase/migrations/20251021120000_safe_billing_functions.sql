...
-- Observação: public.plans.billing_cycle geralmente é TEXT; aplicar CAST explícito.
...
BEGIN
  -- Blindagem: apenas service_role (em Edge/PostgREST haverá JWT com a claim 'role')
  IF coalesce((auth.jwt() ->> 'role'),'') <> 'service_role' THEN
    RAISE EXCEPTION 'forbidden: only service_role can call this function';
  END IF;

  -- Mapeamento/validação cruzada pelo catálogo
