-- 20251103_070500_seed_products.sql
-- Seed: 10 produtos padrão (multi-tenant, idempotente)
-- Logs: [SEED] [PRODUTOS]
...
begin
  ...
  -- Payload base
  with payload(sku, nome, preco, unidade, status, descricao) as (
    values
      ('PROD-001','Camiseta Algodão Pima',           89.90, 'UN', 'ativo', 'Camiseta premium 100% algodão pima'),
      ('PROD-002','Calça Jeans Slim',               189.90, 'UN', 'ativo', 'Modelagem slim, lavagem média'),
      ('PROD-003','Tênis de Corrida Leve',         299.50, 'UN', 'ativo', 'Entressola responsiva'),
      ('PROD-004','Mochila Urbana Impermeável',    150.00, 'UN', 'ativo', 'Compartimento para notebook 15.6"'),
      ('PROD-005','Garrafa Térmica Inox 500ml',     75.00, 'UN', 'ativo', 'Parede dupla, mantém 12h'),
      ('PROD-006','Fone Bluetooth TWS',            250.00, 'UN', 'ativo', 'AAC, estojo com carga rápida'),
      ('PROD-007','Mouse Sem Fio Ergonômico',      120.00, 'UN', 'ativo', '2.4G, DPI ajustável'),
      ('PROD-008','Teclado Mecânico Compacto',     350.00, 'UN', 'ativo', 'ABNT2, hot-swap'),
      ('PROD-009','Monitor 24" Full HD',           899.90, 'UN', 'ativo', 'IPS, 75Hz, VESA'),
      ('PROD-010','Cadeira Escritório Ergonômica', 999.00, 'UN', 'ativo', 'Apoio lombar, ajuste de altura')
  )
  -- Inserção com enum casts explícitos
  insert into public.produtos (
    empresa_id, nome, sku, preco_venda, unidade, status
    -- 'tipo' só se existir e aceitar valor, caso contrário omitimos
  )
  select
    p_empresa_id,
    p.nome,
