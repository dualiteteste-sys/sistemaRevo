-- [MIGRATION] RPCs: CRUD de Transportadoras
-- Padrões: SECURITY INVOKER para leitura, SECURITY DEFINER para escrita, search_path seguro, grants mínimos.

-- 1) Listagem (SECURITY INVOKER)
...
as $$
  ...
  where (p_status is null or t.status = p_status)
    and (p_q is null or t.nome_razao_social ilike '%' || p_q || '%' or t.cnpj ilike '%' || p_q || '%')
  order by
...
-- 4) Create/Update (SECURITY DEFINER)
...
begin
  if v_empresa_id is null then raise exception 'Nenhuma empresa ativa.'; end if;
  ...
  if v_carrier_id is null then
    ...
    if not found then raise exception 'Transportadora não encontrada ou não pertence à empresa.'; end if;
  end if;
...
begin
  ...
  delete from public.transportadoras where id = p_id and empresa_id = v_empresa_id;
