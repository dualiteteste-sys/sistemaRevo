-- [MIGRATION] Adiciona a RPC update_active_company para atualizar a empresa ativa do usuário.
-- Esta função é segura, usa o contexto do usuário autenticado e retorna um único objeto JSON.

begin;

-- Remove a função antiga se existir, para evitar conflitos de tipo de retorno.
...
begin
  ...
  -- Busca o ID da empresa que está marcada como ativa para o usuário.
  ...
  -- Valida se o usuário é realmente membro da empresa que está tentando modificar.
  if not public.is_user_member_of(v_empresa_id) then
    raise exception 'Acesso negado à empresa ativa.' using errcode = '42501';
  end if;

  -- Atualiza apenas os campos fornecidos no patch, mantendo os valores existentes se não forem passados.
