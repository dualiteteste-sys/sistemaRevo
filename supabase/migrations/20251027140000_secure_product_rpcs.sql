...
begin
  -- Fallback: se o JWT não tiver empresa_id, pega a 1ª empresa do usuário
  ...
  if v_empresa_id is null then
    raise exception '[AUTH] empresa_id ausente no contexto' using errcode = '28000';
  end if;

  -- Hard check de membresia
  if not exists (
    select 1 from public.empresa_usuarios
     where empresa_id = v_empresa_id and user_id = v_user_id
  ) then
    raise exception 'Forbidden: user não é membro da empresa alvo' using errcode = '42501';
  end if;

  -- Validações de NCM/CEST
...
declare
  ...
  v_prod       public.produtos;
begin
  -- Fallback para empresa do usuário
  if v_empresa_id is null then
    select eu.empresa_id
      into v_empresa_id
      from public.empresa_usuarios eu
...
-- Índice para validar/consultar membresia
create index if not exists idx_empresa_usuarios_emp_user
