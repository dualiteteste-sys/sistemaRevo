/*
  # [MIGRATION] CRUD de Parceiros (Pessoas) — Atualização
  Ajustes:
  - Inclui os novos campos (tipo_pessoa, fantasia, etc.) na RPC de create/update.
  - Mantém as outras RPCs (leitura) que se adaptam dinamicamente.
*/
...
begin
  ...
  if v_pessoa_id is null then
    insert into public.pessoas (
      ...
      tipo_pessoa, fantasia, codigo_externo, contribuinte_icms, contato_tags
    )
    values (
      v_empresa_id,
  ...
  if v_has_enderecos then
    ...
    loop
      if nullif(v_endereco->>'id','')::uuid is not null then
        update public.pessoa_enderecos set
          tipo_endereco = v_endereco ->> 'tipo_endereco', logradouro = v_endereco ->> 'logradouro',
          numero = v_endereco ->> 'numero', complemento = v_endereco ->> 'complemento',
          bairro = v_endereco ->> 'bairro', cidade = v_endereco ->> 'cidade',
          uf = v_endereco ->> 'uf', cep = v_endereco ->> 'cep', pais = v_endereco ->> 'pais',
          updated_at = now()
        ...
        insert into public.pessoa_enderecos (
          id, empresa_id, pessoa_id, tipo_endereco, logradouro, numero, complemento, bairro, cidade, uf, cep, pais
        ) values (
          coalesce(nullif(v_endereco->>'id','')::uuid, gen_random_uuid()), v_empresa_id, v_pessoa_id,
    ...
    where pessoa_id = v_pessoa_id and empresa_id = v_empresa_id and (cardinality(v_endereco_ids_in_payload) = 0 or id <> all(v_endereco_ids_in_payload));
  end if;
  ...
  if v_has_contatos then
    ...
    loop
      if nullif(v_contato->>'id','')::uuid is not null then
        ...
        insert into public.pessoa_contatos (
          ...
          coalesce(nullif(v_contato->>'id','')::uuid, gen_random_uuid()), v_empresa_id, v_pessoa_id,
    ...
    where pessoa_id = v_pessoa_id and empresa_id = v_empresa_id and (cardinality(v_contato_ids_in_payload) = 0 or id <> all(v_contato_ids_in_payload));
  end if;
