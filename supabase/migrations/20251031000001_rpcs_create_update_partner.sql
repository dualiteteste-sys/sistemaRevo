-- [RPC FIX] create_update_partner — inclui celular e site (nullable)
...
begin
  ...
  if v_pessoa_id is null then
    ...
    values (
      ...
      nullif(v_pessoa_payload ->> 'celular',''),     -- << novo
      nullif(v_pessoa_payload ->> 'site',''),        -- << novo
    ...
    update public.pessoas set
      ...
      celular          = coalesce(nullif(v_pessoa_payload->>'celular',''), celular),  -- << novo
      site             = coalesce(nullif(v_pessoa_payload->>'site',''), site),        -- << novo
  ...
  -- Endereços: somente se payload é array
  if v_has_enderecos then
    for v_endereco in
      select * from jsonb_array_elements(v_enderecos_payload)
    loop
  ...
  -- Contatos: somente se payload é array
  if v_has_contatos then
    for v_contato in
      select * from jsonb_array_elements(v_contatos_payload)
    loop
