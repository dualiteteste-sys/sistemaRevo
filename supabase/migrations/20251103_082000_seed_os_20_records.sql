-- 20251103_082000_seed_os_20_records.sql (FIX v2)
-- Seed: cria até 20 O.S. por empresa (idempotente e seguro)
-- Logs: [SEED][OS]

drop function if exists public.seed_os_for_current_user();

create function public.seed_os_for_current_user()
...
begin
  ...
  -- Garante dados-base idempotentes
  ...
  -- Loop: cria até 20 O.S. (idempotente por descrição-âncora)
  for v_i in 1..20 loop
    v_desc := 'Manutenção Preventiva - Exemplo ' || v_i;

    -- pular se já existir no tenant
    if exists (
      select 1 from public.ordem_servicos os
       where os.empresa_id = v_emp
         and os.descricao = v_desc
    ) then
      continue;
    end if;

    -- Seleção determinística por OFFSET (módulo do total)
    ...
    -- Itens (usam RPCs existentes com recálculo)
    ...
    -- retorna a OS criada para o caller
    return query
      select * from public.ordem_servicos where id = v_os_id;

    perform pg_notify('app_log', '[SEED][OS] criada ' || v_os_id::text);
  end loop;

  perform pg_notify('app_log', '[SEED][OS] verificação/conclusão para 20 registros.');
end;
$$;
